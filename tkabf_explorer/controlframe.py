...
class ControlFrame(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.current_dir_listbox = tk.Listbox(
            self, font=controller.small_font, width=25, height=1, bd=0, relief="flat"
        )
        self.choose_dir_button = ttk.Button(
            self, text="Choose a directory", command=self.open_dir
        )
        self.abf_path_dict = {}
        # setup scrollbox for files
        self.list_of_files_scrollbox = tk.Listbox(
            self, width=25, height=25, font=controller.large_font, exportselection=False
        )  # , selectmode=tk.MULTIPLE) # for future implementation
        ...
        self.list_of_files_scrollbox.config(
            yscrollcommand=self.list_of_files_scrollbar.set
        )
        # layout

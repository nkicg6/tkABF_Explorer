...
class PlotFrame(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.instructions_label = tk.Label(
            ...
            text="Click and drag on the top plot to zoom.",
        )
    ...
    def update_plot(self):
        ...
        self.ax1.cla()
        self.ax2.cla()
        for y1, y2, sweep_label in zip(
            self.current_plot_options["y1"],
            self.current_plot_options["y2"],
            self.current_plot_options["sweep_label"],
        ):
            self.ax1.plot(
                self.current_plot_options["x"],
                y1,
                label=sweep_label,
            ...
            self.ax2.plot(
                self.current_plot_options["x"],
                y2,
                color=self.current_plot_options["y2_color"],
                lw=self.current_plot_options["linewidth"],
            )
        ...
        if self.xlims and self.ylims is not None:
            ...
            self.ax1.set_ylim(self.ylims)
        self.figcanvas.draw()
        self.RS = RectangleSelector(self.ax1, self.boxzoom, drawtype="box")
